...
import gva.ydh.com.util.FileUtil;
...
public class CrashHandler implements UncaughtExceptionHandler {
     ...
     */
    public CrashHandler(GvaApp application) {
...
//                        restartIntent); // 1秒钟后重启应用   
    ...
    private boolean handleException(final Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(application.getApplicationContext(), "应用发生异常，即将退出！000", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }.start();
        // 保存错误报告文件
